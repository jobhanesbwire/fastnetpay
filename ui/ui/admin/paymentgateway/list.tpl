                            ...
                            {foreach $pgs as $pg}
                                <tr>
                                    ...
                                    <td><a href="{Text::url('paymentgateway/')}{$pg}"
                                            class="btn btn-block btn-{if in_array($pg, $actives)}info{else}default{/if} text-left">{if $pg eq 'mpesastkpush'}M-Pesa STK Push{else}{ucwords(str_replace('_', ' ', $pg))}{/if}</a>
                                    </td>
                                    <td width="114">
